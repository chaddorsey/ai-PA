...
- **Enrichment layers** (`enrich.py`, build-time): adds features to `route_guide.json` from external sources, projected onto the milepost axis — **towns/cities** (GeoNames), **county-traversal `area` spans with demographics/occupations/socioeconomics** (Census ACS Data Profiles; needs `CENSUS_API_KEY` in `.env`), **agricultural land use** — top crops per county (USDA NASS Census of Agriculture; needs `NASS_API_KEY`), **land cover** — per-county mix + a continuous along-track profile of forest/cropland/developed/shrub/water/wetland (USGS NLCD via MRLC WMS, keyless), and an **elevation profile + auto-detected summits/grades** (open-meteo). The `profile` command surfaces the area data ("who lives here / what they do"); display beyond that (choropleths, app cards) is deferred. Each enricher is idempotent; downloads cache in `.cache/`. Pipeline: `build_route_guide.py` (Phase A) → `enrich.py towns counties acs elevation`. Design + phasing: `docs/plans/2026-06-26-amtrak-route-guide-build.md` (Phases B/C/E).
- **Google Earth fly-ahead** (`kml_tour.py`): a renderer on the route-guide contract — the polyline becomes a flight path, features become placemarks, and marquee sights become timed camera stops in a `gx:Tour`. Each feature and camera is stamped with the **predicted (typical) clock time** you'd be there on the real July dates (from `eta_to_mile`) and the **modeled sun** at that time/place (`sun.py`): elevation, compass direction, and which window it's on — e.g. *"Glacier NP — Jul 13, 9:16 PM, sun WNW 2° golden hour"* or *"Pecos High Bridge — 12:24 AM, night (you'll sleep through it)"*. The KML `<TimeStamp>`s also drive Google Earth's own **Sunlight** feature (enable "Sunlight across the landscape") so the sun renders correctly as the tour plays. `python3 kml_tour.py all` writes one `.kml` per leg into `kml/` (gitignored, regenerable).
