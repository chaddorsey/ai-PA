...
python3 enrich.py towns counties acs nass elevation   # Phases B/C/E: towns, county demographics, land use, elevation
...
- **Enrichment layers** (`enrich.py`, build-time): adds features to `route_guide.json` from external sources, projected onto the milepost axis — **towns/cities** (GeoNames), **county-traversal `area` spans with demographics/occupations/socioeconomics** (Census ACS Data Profiles; needs `CENSUS_API_KEY` in `.env`), **agricultural land use** — top crops per county (USDA NASS Census of Agriculture; needs `NASS_API_KEY`), and an **elevation profile + auto-detected summits/grades** (open-meteo). The `profile` command surfaces the area data ("who lives here / what they do"); display beyond that (choropleths, app cards) is deferred. Each enricher is idempotent; downloads cache in `.cache/`. Pipeline: `build_route_guide.py` (Phase A) → `enrich.py towns counties acs elevation`. Design + phasing: `docs/plans/2026-06-26-amtrak-route-guide-build.md` (Phases B/C/E).
