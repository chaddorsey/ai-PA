...
metadata: {"clawdbot":{"emoji":"👥","requires":{"bins":["letta-teams"]}}}
---
...
All `letta-teams` commands MUST be prefixed with environment variables for the self-hosted Letta server:
```bash
LETTA_API_KEY=self-hosted LETTA_BASE_URL=http://localhost:8283 letta-teams <command>
```
The daemon runs as a launchd service (`com.ai-pa.letta-teams-daemon`) on port 9774.
Working directory: `/Volumes/main-drive/ai-PA`
