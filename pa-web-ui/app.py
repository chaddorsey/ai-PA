...
# (pa-routing-handler was decommissioned 2026-05-30; ROUTING_HANDLER_URL
# constant removed. /api/agents now serves the static FLEET_AGENT_NAMES.)
...
def get_agents():
    """Return the static fleet agent list for the chat header picker.

    Previously proxied to pa-routing-handler /v1/agents. After
    pa-routing-handler decommission (2026-05-30), this serves the
    same shape directly from FLEET_AGENT_NAMES — single source of
    truth lives near the subprocess dispatcher below.
    """
    agents = [{
        "id": "",
        "name": "Auto (Recommended)",
        "description": "Default to Mission Control; per-message slash routing still supported.",
        "keywords": ["auto", "smart"],
    }]
    # FLEET_AGENT_NAMES is defined later in this file. Reference it lazily
    # via globals() so module initialization order doesn't matter.
    fleet = globals().get("FLEET_AGENT_NAMES", {})
    for agent_id, name in fleet.items():
        agents.append({
            "id": agent_id,
            "name": name,
            "description": name,
...
# Friendly names for fleet agents. This is the SOURCE OF TRUTH for both
# the chat header picker (served via /api/agents) and the subprocess pool
# dispatcher below. Previously synced with pa-routing-handler; that
# service was decommissioned 2026-05-30.
...
def stream():
    ...
    Receives a message, dispatches to the appropriate fleet agent via
    the subprocess pool, and streams the response back via SSE.

    The legacy pa-routing-handler /v1/route + /v1/coordinate paths
    were decommissioned 2026-05-30 — coordination is now an MC skill
    rather than a pre-dispatch microservice.
    """
    data = request.get_json(force=True, silent=True) or {}
    ...
    # Legacy pa-routing-handler /v1/route + /v1/sessions/.../complete path
    # was decommissioned 2026-05-30. All fleet agents now dispatch via the
    # subprocess pool earlier in this function. Any caller reaching here is
    # passing an agent_id that's not in FLEET_AGENT_NAMES — surface that
    # explicitly with a 410 rather than silently failing.
    logger.warning(
        "stream_legacy_route_path_hit",
        session_id=session_id,
        agent_id=agent_id,
        slash_command=slash_command,
    )
    return jsonify({
        "error": (
            "Legacy /v1/route path was decommissioned 2026-05-30. "
            "All fleet agents dispatch via subprocess pool; non-fleet agents "
            "are no longer supported via /stream."
        )
    }), 410
