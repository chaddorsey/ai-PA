...
def notify_agent_new_meeting(meeting_id: str, meeting_title: str):
    ...
    message_content = (
        ...
        f"Step 1: call scan_meeting_notes(meeting_id=\"{meeting_id}\"). This "
        f"writes [c] marker rows to pa_web.task_queue (source='meeting' or "
        f"'meeting_marker') in cycle 1 — the legacy queued_tasks_from_meetings "
        f"BLOCK is retired.\n"
        f"Step 2: Review the scan package. Merge marker extractions with "
        f"semantic findings.\n"
        f"Step 3: call prepare_meeting_followup with merged results (creates "
        f"Gmail draft).\n"
        f"Step 4: call consume_queue(source='meeting', limit=20) AND "
        f"consume_queue(source='meeting_marker', limit=20) to claim the rows "
        f"this meeting just produced. For each claimed row, call "
        f"add_extracted_tasks_postgres(...) to land it in pa_web.tasks — the "
        f"enrichment-scanner picks up enrichment_state='pending' rows and "
        f"runs the 4-tool chain. Do NOT call the legacy add_extracted_tasks "
        f"or process_spark_queue — those paths were retired in cycle 1.\n"
        f"DUPLICATE-CHECK POLICY: rely on the DB's ON CONFLICT (ref_id) DO "
        f"NOTHING — if you suspect a duplicate, attempt the insert anyway "
        f"and trust the tool's status='exists' vs 'ok' return. Do not skip "
