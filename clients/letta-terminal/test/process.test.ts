...
/**
 * Every character in `text` that a terminal would ACT on rather than display.
 *
 * A codepoint scan rather than a regex of literal control characters: the regex was unreadable in
 * a diff, and it returns the offending characters themselves, so a failure names what leaked
 * instead of just asserting that something did. Newline and tab are content and are kept — the
 * sanitizer preserves them deliberately.
 */
function terminalActionableChars(text: string): string[] {
  return [...text].filter((ch) => {
    if (ch === "\n" || ch === "\t") return false;
    const code = ch.codePointAt(0) ?? 0;
    // C0, DEL, and the C1 block — which includes the 8-bit CSI/OSC forms that bypass any
    // ESC-anchored filter.
...
describe("the CLI as a real process", () => {
  ...
  describe("the harness itself", () => {
    ...
    it(
      "reports a clean exit, keeps stdout and stderr apart, and returns promptly",
      async () => {
    ...
    it(
      "reports the CLI's own status, not the last pipeline stage's",
      async () => {
    ...
    it(
      "treats a process that will not exit as a FAILURE, with the evidence attached",
      async () => {
  ...
  describe("headless attach", () => {
    it(
      "exits instead of hanging when stdin is /dev/null",
      async () => {
  ...
  describe("a closed stdout", () => {
    it(
      "does not launder a failing run into exit 0",
      async () => {
    ...
    it(
      "still leaves quietly when the consumer goes away on a SUCCEEDING run",
      async () => {
  ...
  describe("hostile argv", () => {
    it(
      "never echoes a control sequence back to the terminal from the arg-parse error path",
      async () => {
        // D1. The argument-parse error path wrote `err.message` UNSANITIZED while every sibling
        // diagnostic sanitized, and the message embeds argv verbatim. So
        // `letta-continuity $'\x1b]52;c;…\x07'` came back as `unknown option: <ESC>]52;…` — an
        // OSC-52 clipboard write, performed by the operator's own terminal, before any socket
        // opens. Agents write this repo's env and invoke this binary, so the argv is not
        // necessarily the operator's.
        //
        // Driven as a real process because that is the only place argv is really argv; the
        // in-process harness passes an array that never crosses a shell.
        const r = await runCli(["--]52;c;cGF5bG9hZA==oops"], {
          timeoutMs: RUN_TIMEOUT_MS,
        });

        expect(r.timedOut).toBe(false);
        expect(r.cliExitCode).toBe(2);
        // Nothing a terminal acts on: no ESC, no 8-bit CSI/OSC, no DEL, no other C0/C1.
        expect(terminalActionableChars(r.stderr)).toEqual([]);
        // …but it still says what went wrong, or sanitizing would just be silencing.
        expect(r.stderr.toLowerCase()).toContain("unknown option");
      },
      TEST_TIMEOUT_MS,
    );

    it(
      "never echoes a control sequence from a hostile --url",
      async () => {
        // The same path reached through the trust boundary rather than the option parser: a
        // non-loopback URL is refused, and the refusal used to quote the URL verbatim.
        const r = await runCli(["--url", "ws://]0;pwnedevil.example/ws"], {
          timeoutMs: RUN_TIMEOUT_MS,
        });
  ...
  describe("on a pty", () => {
    it(
      "runs a one-shot to completion on a real terminal",
      async () => {
