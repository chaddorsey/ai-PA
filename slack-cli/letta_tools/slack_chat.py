...
def run_slack_chat(action: str, params: Optional[str] = None, fields: Optional[str] = None) -> Dict[str, Any]:
    """
    Send, update, and delete Slack messages. Run `slack schema chat.<action>` to discover params.

    Args:
        action: One of: postMessage, update, delete, postEphemeral, scheduleMessage, unfurl, +send (REQUIRED)
