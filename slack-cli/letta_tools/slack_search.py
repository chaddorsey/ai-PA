...
def run_slack_search(action: str, params: Optional[str] = None, fields: Optional[str] = None) -> Dict[str, Any]:
    """
    Search Slack messages and files. Requires user token. Run `slack schema search.<action>` to discover params.

    Args:
        action: One of: messages, files (REQUIRED)
        params: JSON string with parameters. Use `slack schema search.<action>` to discover fields.
