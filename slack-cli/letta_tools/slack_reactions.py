...
def run_slack_reactions(action: str, params: Optional[str] = None, fields: Optional[str] = None) -> Dict[str, Any]:
    """
    Add, remove, and list Slack reactions. Run `slack schema reactions.<action>` to discover params.

    Args:
        action: One of: add, remove, get, list (REQUIRED)
        params: JSON string with parameters. Use `slack schema reactions.<action>` to discover fields.
    ...
    try:
        cli_args = ["slack", "reactions", action]
        if params:
