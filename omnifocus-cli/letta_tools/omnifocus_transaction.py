...
def omnifocus_transaction(action: str, params: Optional[str] = None, fields: Optional[str] = None) -> Dict[str, Any]:
    """
    OmniFocus transaction operations: batch operations with rollback support. Run omnifocus-cli schema transaction.<action> to discover params.

    Args:
        action: One of: begin, execute, accept, rollback, history (REQUIRED)
    ...
    try:
        cli_args = ["omnifocus-cli", "transaction", action]
        if params:
