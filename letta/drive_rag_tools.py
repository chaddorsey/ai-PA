...
def analyze_entity_consolidation(
    similarity_threshold: Optional[float] = None,
) -> Dict[str, Any]:
    """
    Analyze entities for potential duplicates and consolidation opportunities.

    Use this tool to review the knowledge graph for entities that may be
    duplicates and should be merged. For example:
    - "NSF" and "National Science Foundation" are the same entity
    - "Nathan Kimball" and "N. Kimball" may refer to the same person
    - "UIUC" and "University of Illinois" are the same organization

    This helps maintain a clean, normalized knowledge graph.

    Args:
        similarity_threshold: Minimum similarity score (0.0 to 1.0) to flag
                              as a potential duplicate. Default is 0.6.
                              Higher values = fewer, more confident matches.

    Returns:
        Dictionary with consolidation report including:
        - summary: Total entities and type counts
        - clusters: Groups of potentially duplicate entities
    ...
    try:
        ...
        # Make request
        response = requests.get(
            f"{base_url}/v1/entities/consolidation",
        ...
        if response.status_code != 200:
            return {
                "status": "error",
                "error_message": f"Failed to analyze entities: {response.text}",
            }
        ...
        return {
            ...
            "clusters": data.get("clusters", [])[:10],  # Limit to first 10 for readability
