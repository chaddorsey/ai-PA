...
def get_drive_file_info(drive_url: str) -> str:
    ...
    try:
        ...
        if not file_id:
            # Check if the URL itself is just an ID (no slashes, dots, etc.)
        ...
        # Load credentials and query Drive API
        creds = _load_credentials()
        service = build("drive", "v3", credentials=creds)
...
def search_drive_activity(
    ...
    try:
        ...
        # Fetch links for top documents
        from googleapiclient.discovery import build
        from googleapiclient.errors import HttpError
        
        service = build("drive", "v3", credentials=creds)
        for doc in doc_list[:25]:  # Limit API calls
            try:
                file = service.files().get(
                    fileId=doc["doc_id"],
...
def get_drive_documents(
    ...
    try:
        ...
        if owner:
            ...
            owner_queries = []
            for o in owner_list:
                # Handle partial email (ending with @)
