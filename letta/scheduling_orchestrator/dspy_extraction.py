...
if DSPY_AVAILABLE:
    class ExtractSchedulingRequest(dspy.Signature):
        ...
        """
        utterance: str = dspy.InputField(desc="Natural language scheduling request. When the utterance says 'me' or 'I', it refers to the requester (first participant in context). When the utterance says 'with X and Y', the requester should be included in the participants list. Extract participant preferences: If utterance mentions 'X prefers mornings' or 'X likes Tuesday', extract as participant preference. If utterance mentions 'avoid Friday' or 'not on Monday', extract as avoid preference. If utterance mentions 'flexible meetings' or 'moveable events', extract as flexibility notes. RESCHEDULING DETECTION: CRITICAL - Do NOT set is_rescheduling=true for phrases like 'Schedule a new meeting', 'Create a new meeting', 'Set up a new meeting', 'Book a new meeting' - these are NEW meeting requests, not rescheduling. Only set is_rescheduling=true if the utterance contains rescheduling keywords like 'reschedule', 'find new time', 'move', 'change time', 'new time for', 'different time', 'alternative time', AND the utterance does NOT contain phrases indicating a new meeting. DUAL-TIMEFRAME PATTERN FOR RESCHEDULING: Rescheduling queries inherently contain TWO time-based referents: (1) EVENT IDENTIFICATION timeframe - when the original event occurs (e.g., 'Wednesday afternoon', 'Dec. 10th at 2pm', 'tomorrow morning'), and (2) SEARCH WINDOW timeframe - when to look for new slots (e.g., 'next week', 'this month', 'in the next two weeks'). CLASSIFICATION RULES: (a) If TWO time references exist: The more SPECIFIC one (date+time, day+time, or specific date) goes to event_identifiers (to find the existing event). The more GENERAL one (timeframe phrases like 'next week', 'this month', 'in two weeks') indicates search window preference (can be noted but time_window will come from context_json). (b) If ONE time reference exists: If SPECIFIC (date+time, day+time, specific date like 'Dec. 10th') → event_identifiers. If GENERAL (timeframe like 'next week', 'this month') → search window preference. Extract event identifiers: participant names mentioned in context of the event (e.g., 'meeting with Judi'), dates (e.g., 'on Dec. 10th', 'Wednesday', 'tomorrow'), times (e.g., 'at 2pm', 'in the morning', 'afternoon'), and event titles (e.g., 'check-in', 'standup', 'review meeting'). CRITICAL TITLE EXTRACTION: Event titles are typically noun phrases that describe the meeting purpose (e.g., 'Concord Audit Drafts', 'Support Team Meeting', 'Grants Team Meeting', 'Weekly Standup'). Titles often contain capitalized words, may include organization names, and are usually mentioned after 'the' or 'for the' (e.g., 'for the Concord Audit Drafts meeting'). Do NOT extract meeting titles as participant names. Participant names are typically personal names (e.g., 'Judi', 'Chad', 'Kate') or appear after 'with' or 'and' (e.g., 'meeting with Judi and Chad'). If a phrase like 'Concord Audit Drafts' appears in the utterance, it is almost certainly a meeting title, not participant names. CRITICAL: When is_rescheduling is true, ALWAYS set time_window_start and time_window_end to null (the full timeframe from context_json will be used). Extract date/time preferences as preferred_days/preferred_times only if they represent preferences for NEW slots (not event identification). Example: 'Wednesday afternoon check-in next week' → event_identifiers={dates:['Wednesday'], times:['afternoon'], titles:['check-in']}, preferred_days:null, preferred_times:null, time_window_start:null, time_window_end:null (the 'next week' is understood from context_json timeframe). Example: 'Find alternative times for the Concord Audit Drafts meeting' → event_identifiers={titles:['Concord Audit Drafts']}, participant_names:[], dates:[], times:[].")
        ...
        problem_json: str = dspy.OutputField(desc="Valid JSON object (not a string) matching SchedulingProblem schema. Required fields: participants (list of email addresses/IDs from context - MUST include the requester if utterance uses 'with', 'me', or 'I'), duration_minutes (integer), time_window_start (ISO 8601 UTC string or null), time_window_end (ISO 8601 UTC string or null), preferred_times (list of ISO strings or null), preferred_days (list like ['Monday', 'Tuesday'] or null), participant_preferences (array of {participant_id, preferred_times, preferred_days, avoid_times, avoid_days, flexibility_notes} or null), avoid_times (array of ISO 8601 UTC strings for times to avoid or null), avoid_days (array of day names to avoid like ['Friday'] or null), title (string or null), location (string or null), min_gap_minutes (integer or null), allow_off_hours (boolean). Optional rescheduling fields: is_rescheduling (boolean, true if utterance contains rescheduling keywords like 'reschedule', 'find new time', 'move', 'change time'), event_identifiers (object with participant_names: array of participant names mentioned in event context, dates: array of date references like 'Dec. 10th' or 'Wednesday', times: array of time references like '2pm' or 'afternoon', titles: array of event title keywords like 'check-in' or 'standup'). CRITICAL FOR RESCHEDULING - DUAL-TIMEFRAME CLASSIFICATION: When is_rescheduling is true, you MUST distinguish between: (1) EVENT IDENTIFICATION time references (specific dates/times to find the existing event) → extract to event_identifiers.dates and event_identifiers.times, (2) SEARCH WINDOW time references (general timeframes like 'next week') → these are handled by context_json timeframe, do NOT extract as time_window. ALWAYS set time_window_start and time_window_end to null for rescheduling (the full timeframe from context_json will be used). Only extract preferred_days/preferred_times if the utterance explicitly expresses preferences for NEW time slots (e.g., 'prefer mornings', 'any day except Friday'). Do NOT extract preferred_days/preferred_times from event identification time references. Map participant names to their email addresses from the context. Always include the requester when utterance contains 'with' or mentions 'me'/'I'.")
else:
...
def extract_scheduling_request(
    ...
    try:
        ...
        # Fix None values before Pydantic validation
        # DSPy may return null for duration_minutes, which Pydantic rejects
        if problem_data.get("duration_minutes") is None:
            problem_data["duration_minutes"] = 45  # Default duration for rescheduling requests
        
        # Filter out invalid participants (common words)
        if "participants" in problem_data and isinstance(problem_data["participants"], list):
            invalid_participants = {"the", "a", "an", "this", "that", "these", "those", "me", "my", "mine", "you", "your", "yours", "we", "our", "ours", "they", "their", "theirs", "it", "its", "what", "which", "who", "whom", "whose", "where", "when", "why", "how", "is", "are", "was", "were", "be", "been", "being", "have", "has", "had", "do", "does", "did", "will", "would", "could", "should", "may", "might", "must", "can", "find", "me", "next", "grants", "team", "meeting", "review", "planning", "sync", "check-in", "options", "times", "time", "there", "timeslot"}
            problem_data["participants"] = [p for p in problem_data["participants"] if p and p.lower() not in invalid_participants]
