...
if DSPY_AVAILABLE:
    class ExtractSchedulingRequest(dspy.Signature):
        ...
        """
        utterance: str = dspy.InputField(desc="Natural language scheduling request. When the utterance says 'me' or 'I', it refers to the requester (first participant in context). When the utterance says 'with X and Y', the requester should be included in the participants list. Extract participant preferences: If utterance mentions 'X prefers mornings' or 'X likes Tuesday', extract as participant preference. If utterance mentions 'avoid Friday' or 'not on Monday', extract as avoid preference. If utterance mentions 'flexible meetings' or 'moveable events', extract as flexibility notes. RESCHEDULING DETECTION: If the utterance contains rescheduling keywords like 'reschedule', 'find new time', 'move', 'change time', 'new time for', 'different time', set is_rescheduling to true and extract event identifiers: participant names mentioned in context of the event (e.g., 'meeting with Judi'), dates (e.g., 'on Dec. 10th', 'Monday', 'tomorrow'), times (e.g., 'at 2pm', 'in the morning'), and event titles (e.g., 'check-in', 'standup', 'review meeting'). IMPORTANT FOR RESCHEDULING: When is_rescheduling is true, do NOT extract narrow time_window_start/time_window_end from date/time references in the utterance. Instead: (1) Extract date/time references as event_identifiers (to identify the existing event), (2) Extract date/time preferences as preferred_days/preferred_times (for the new time slots), (3) Set time_window_start and time_window_end to null (the full timeframe will be used from context_json). For example, 'Wednesday afternoon check-in next week' should extract: event_identifiers={dates:['Wednesday'], times:['afternoon'], titles:['check-in']}, preferred_days:['Wednesday'], preferred_times:['afternoon'], time_window_start:null, time_window_end:null.")
