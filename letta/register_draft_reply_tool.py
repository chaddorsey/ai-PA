#!/usr/bin/env python3
"""
Register draft_reply_to_email Tool with Letta

Registers the tool and attaches it to tasks-agent-sleeptime.
Designed to be run once from the host.

Usage:
    LETTA_BASE_URL=http://localhost:8283 python letta/register_draft_reply_tool.py
"""
...
from draft_reply_to_email_tool import draft_reply_to_email

TOOL_NAME = "draft_reply_to_email"
...
def main():
    ...
    created_tool = client.tools.create_from_function(
        func=draft_reply_to_email,
        tags=["gmail", "email", "draft", "reply"],
    )
