...
server.tool(
  "listRemaining",
  "Return all of your incomplete OmniFocus tasks",
  {},
  async () => {
...
server.tool(
  "listProjects",
  "Return all your OmniFocus projects",
  {},
  async () => {
...
server.tool(
  "listTags",
  "Return all your OmniFocus tags",
  {},
  async () => {
...
server.tool(
  "listTasksByProject",
  "Return all tasks inside a specific OmniFocus project (including nested tasks)",
  { projectId: z.string().describe("The project’s UUID") },
  async ({ projectId }) => {
...
server.tool(
  "listTasksByTag",
  "Return all tasks with a specific OmniFocus tag (context)",
  {
...
server.tool(
  "processInboxItem",
  "Process an inbox item by performing multiple operations in a single call (assign project, add tags, set dates, rename, add notes, flag, set duration, or delete). ALREADY ATOMIC - no need to wrap in transaction. Use directly for inbox processing.",
  {
...
server.tool(
  ...
  "Get comprehensive context for intelligent bulk inbox processing including pre-analyzed items, user patterns, suggestions, and available destinations. Use this BEFORE bulk processing to gather all intelligence the AI needs to make smart decisions efficiently.",
  {
...
server.tool(
  ...
  "Execute multiple inbox operations in a single efficient batch with comprehensive error handling, validation, and undo capabilities. Supports processing, merging, holding, and deleting multiple items with atomic transaction support.",
  {
...
server.tool(
  "listPerspectives",
  "Return all OmniFocus perspectives (both built-in and custom) with metadata",
  {},
  async () => {
...
server.tool(
  "switchToPerspective",
  "Switch OmniFocus to a specific perspective (by ID or name)",
  {
...
server.tool(
  ...
  "Get all tasks currently visible in a specific perspective (or current perspective if none specified)",
  {
...
server.tool(
  "getPerspective",
  "Get detailed information about a specific perspective including configuration details for custom perspectives with full binary plist parsing",
  {
        ...
        if (parsedPlist && parsedPlist.length > 0) {
          ...
          const organizedConfig = {
            ...
            enhancements: {
              ...
              nextStepsNeeded: [
                ...(parsedFilterRules.some((rule: any) => rule.actionWithinFocus || rule.actionHasAllOfTags || rule.actionHasAnyOfTags) ? 
          ...
          if (parsedFilterRules && Array.isArray(parsedFilterRules)) {
            const filterDetails = parsedFilterRules.map((rule: any) => {
                ...
                switch (key) {
                  ...
                  case 'aggregateRules':
                    ...
                    if (Array.isArray(value) && rule.aggregateType) {
                      const ruleDescriptions = value.map((subRule: any) => {
...
server.tool(
  "listFolders",
  "Return folders with optional filtering and hierarchy navigation",
  {
...
server.tool(
  "getFolderHierarchy",
  "Return complete folder hierarchy with optional project inclusion",
  {
...
server.tool(
  "getProjectsByFolder",
  "Return all projects within a specific folder with filtering and sorting options",
  {
...
server.tool(
  "getProjectPath",
  "Return the complete folder path and context information for a specific project",
  {
...
server.tool(
  "queryTasks",
  "Query OmniFocus tasks with advanced multi-dimensional filtering including exclusions and metadata-based filters",
  {
...
server.tool(
  "searchTasks",
  "Search tasks with full-text matching, exclusion filtering, date/duration/status filters, and advanced metadata filters. Returns scored results with match type information.",
  {
...
server.tool(
  "universalQuery",
  "Universal query interface combining search and structured filtering with intelligent optimization. Supports all searchTasks and queryTasks capabilities in a single optimized method.",
  {
    // Search parameters
...
server.tool(
  "createFolder",
  "Create a new folder in OmniFocus with optional parent folder and positioning",
  {
...
server.tool(
  "validateFolderName",
  "Validate a folder name before creation, checking for duplicates and invalid characters",
  {
...
server.tool(
  "getFolderById", 
  "Get detailed information about a specific folder by its UUID",
  {
...
server.tool(
  "moveProject",
  "Move a project between folders with optional position control",
  {
...
server.tool(
  "validateProjectMove",
  "Validate a project move before execution, checking for errors and redundant moves",
  {
...
server.tool(
  "deleteFolder",
  "Delete a folder with safe handling of contained projects. Requires explicit confirmation to prevent accidental data loss.",
  {
...
server.tool(
  ...
  "Validate folder deletion before execution, checking for contained projects and subfolders",
  {
...
server.tool(
  "getFolderProjects",
  "Get detailed information about all projects contained within a folder",
  {
...
server.tool(
  "createProject",
  "Create a new project with optional folder assignment, positioning, and property setting",
  {
...
server.tool(
  ...
  "Validate project creation parameters before execution, checking for duplicates and invalid properties",
  {
...
server.tool(
  ...
  "Convert an existing task into a project with optional folder placement and subtask preservation",
  {
...
server.tool(
  ...
  "Validate task-to-project conversion before execution, checking for constraints and potential issues",
  {
...
server.tool(
  "getTaskSubtasks",
  "Get detailed information about all subtasks of a specific task",
  {
...
server.tool(
  "moveTask",
  "Move a task between projects or within project hierarchy while preserving relationships. USE DIRECTLY for single task moves ONLY. For multiple moves, hierarchy restructuring, or complex reorganization, use restructureTaskHierarchy or executeTransactional(restructureTaskHierarchy) instead - do NOT chain multiple moveTask calls.",
  {
...
server.tool(
  "validateTaskMove",
  "Validate task movement before execution, checking for constraints and relationship integrity",
  {
...
server.tool(
  ...
  "Get detailed information about all relationships of a specific task (children, dependencies, etc.)",
  {
...
server.tool(
  "createSubtask",
  "Create a new subtask under an existing parent task with optional properties and positioning",
  {
...
server.tool(
  "getTaskHierarchy",
  "Get the complete hierarchy tree of a task and all its descendants with full task details",
  {
...
server.tool(
  "createSubtaskHierarchy",
  "Create multiple subtasks at once with support for multi-level hierarchies and complex parent-child relationships",
  {
...
server.tool(
  ...
  "Validate subtask creation parameters before execution, checking parent task status and property validity",
  {
...
server.tool(
  ...
  "Flatten a task hierarchy with configurable flattening behavior - either to direct children or parent level. MODERATE-RISK: Consider using executeTransactional for rollback safety when flattening large hierarchies or when user requests safety.",
  {
...
server.tool(
  "moveTaskBranch",
  "Move an entire task branch (task and all its descendants) to a new location. USE THIS for moving task branches with children instead of multiple moveTask calls. MODERATE-RISK: Consider executeTransactional wrapper for rollback safety.",
  {
...
server.tool(
  "nestTasksAsHierarchy",
  "Convert flat tasks into a hierarchical structure based on provided hierarchy definition",
  {
...
server.tool(
  "previewHierarchyOperation",
  "Preview the effects of a hierarchy operation without executing it",
  {
    operation: z.object({
...
server.tool(
  ...
  "Validate hierarchy integrity and detect issues like circular dependencies, orphaned tasks, and invalid relationships",
  {
...
server.tool(
  "validateOperation",
  "Validate a hierarchy operation before execution to prevent invalid operations and assess risk",
  {
    operation: z.object({
...
server.tool(
  "getValidationErrors",
  "Get detailed validation errors and analysis for specific hierarchies or all hierarchies",
  {
...
server.tool(
  "repairHierarchyIntegrity",
  "Automatically repair hierarchy integrity issues with safety controls and dry-run support",
  {
...
server.tool(
  "getProjectGroupType",
  "Get the group type (parallel/sequential) for a project's tasks",
  {
...
server.tool(
  "setProjectGroupType",
  "Set whether project tasks complete in parallel (independently) or sequential (dependency chain) order",
  {
    projectName: z.string().optional().describe("Project name (use either projectName or projectId)"),
    projectId: z.string().optional().describe("Project UUID (use either projectName or projectId)"),
...
server.tool(
  ...
  "Get whether a project auto-completes when the last task is done or requires manual completion",
  {
    projectName: z.string().optional().describe("Project name (use either projectName or projectId)"),
    projectId: z.string().optional().describe("Project UUID (use either projectName or projectId)")
  },
...
server.tool(
  ...
  "Set project completion behavior - whether it auto-completes with last task or requires manual completion",
  {
    projectName: z.string().optional().describe("Project name (use either projectName or projectId)"),
    projectId: z.string().optional().describe("Project UUID (use either projectName or projectId)"),
...
server.tool(
  "getTaskGroupType", 
  "Get the group type (parallel/sequential) for a parent task's subtasks. Only works on tasks that have subtasks.",
  {
...
server.tool(
  "setTaskGroupType",
  "Set whether subtasks within a parent task complete in parallel (independently) or sequential (dependency chain) order. Only works on tasks that have subtasks.",
  {
    taskName: z.string().optional().describe("Task name (use either taskName or taskId)"),
    taskId: z.string().optional().describe("Task UUID (use either taskName or taskId)"),
...
server.tool(
  "getProjectHierarchy",
  "Get a flat list of all projects with their folder path context. Returns comprehensive project organization data with folder paths, depths, and project metadata for AI analysis.",
  {
...
server.tool(
  "getProjectTree",
  "Get the complete project organization as a nested tree structure. Returns hierarchical view of folders and projects for sophisticated navigation and analysis. Use this for understanding overall organization structure.",
  {
    includeCompleted: z.boolean().optional().describe("Include completed projects in results (default: false)"),
...
server.tool(
  "beginTransaction",
  "Begin a new transaction for atomic execution of multiple hierarchy operations with rollback support. ADVANCED USE: Prefer executeTransactional for most cases. Use this only when you need manual control over individual operation steps.",
  {
...
server.tool(
  "acceptTransaction",
  "Accept an active transaction, finalizing all changes that are already visible in OmniFocus. Use this when user is satisfied with the results and wants to make the transaction permanent. Changes are already applied - this just marks them as accepted.",
  {
...
server.tool(
  "rollbackTransaction",
  "Rollback an active transaction, undoing all changes that were made during the transaction. Changes will be reverted in OmniFocus and the transaction will be marked as rolled back. Only works on ACTIVE transactions - use immediately after executeTransactional or before acceptTransaction.",
  {
...
server.tool(
  "rollbackRecentTransaction",
  "Rollback a recently completed transaction within the timeout window (default: 10 minutes). USE THIS when user says 'undo that', 'roll back the last change', 'revert those changes', etc. IMPORTANT: This works on ACCEPTED transactions - the transaction being 'accepted' does NOT prevent rollback within the 10-minute timeout period. Changes will be reverted using OmniFocus's undo system. Always inform the user that rollback is available for recent changes.",
  {
...
server.tool(
  ...
  "Get history of transactions with their status, operations, and results",
  {
...
server.tool(
  "validateTransaction",
  "Validate a set of operations before executing them in a transaction. USE BEFORE complex operations to assess risk and provide user with safety information. Always use when user asks to 'check if safe' or before risky multi-step operations.",
  {
    operations: z.array(z.object({
