...
| 21-3 | [ASP encoding (grid, hard constraints)](./21-3.md) | Done | Write the clingo logic for selecting one start slot per request, occupying D slots, no overlaps, windows, work hours, min gaps. |
| 21-4 | [ASP soft constraints & lexicographic objectives](./21-4.md) | Done | Add weak constraints with levels (lexicographic): L1 Protect protected events; L2 minimize total moved minutes; L3 maximize focus blocks; L3 respect participant preferences. |
| 21-5 | [clingo wrapper & model extraction](./21-5.md) | Done | Inside the tool, build the wrapper that compiles facts, grounds, solves, extracts start/2, occurs/2, computes move deltas and objective scores, and collects stats. |
...
| 21-13 | [Replace ASP/clingo with pure Python constraint solver](./21-13.md) | InProgress | Replace ASP/clingo backend with pure Python constraint checking and ranking to eliminate "too many messages" errors. Handles free slot finding and single-meeting moves. Maintains same interface for compatibility. |
| 21-14 | [Validate moved events for all participants](./21-14.md) | Done | Validate that all suggested event moves are conflict-free for all participants of the moved event. Enforce internal-only constraint. Proactively fetch calendars for missing participants. |
