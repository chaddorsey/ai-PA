#!/usr/bin/env python3
"""
Test the orchestrator with Dec 10-19 date range using example_event_data_v2.md
"""
import json
import sys
...
def main():
    ...
    events_by_participant = load_events_from_example_v2(example_file)
        ...
        if isinstance(result, dict):
            ...
            if proposals:
                for i, proposal in enumerate(proposals, 1):
                    print(f"Proposal {i}:")
                    start_str = proposal.get('start', '')
                    end_str = proposal.get('end', '')
                    ...
                    if end_str:
                        try:
                            ...
                            print(f"  End: {et.strftime('%A, %B %d, %Y at %I:%M %p %Z')}")
                        except Exception as e:
    ...
    output_file = Path(__file__).parent / 'test_dec10_19_results.json'
    try:
