# Task 29-4: Create Letta tools for main agent

[Back to task list](./tasks.md)

## Description

Create Letta custom tools for the main agent that enable memory-enriched game context and user interaction.
...
1. **get_current_game_state Tool**
   - Retrieve current game situation from game-state-service
   - Return formatted state (score, clock, situation)
   - Include recent play summary

2. **ask_game_question Tool**
   - Accept natural language question about the game
   - Query insight-engine for context-aware answer
   - Return conversational response

3. **get_player_info Tool**
   - Look up player details from ESPN API
   - Return career highlights, current stats
   - Handle player name variations

4. **explain_play Tool**
   - Get detailed breakdown of a specific play
   - Include strategic context
   - Reference similar plays if relevant

5. **get_game_summary Tool**
   - Return drive-by-drive summary
   - Include key moments and turning points
   - Useful for mid-game catch-up (future)

## Implementation Plan

### File Structure
```
auto-madden/letta-tools/
...
All tools follow Letta tool design patterns:
...
def get_current_game_state() -> Dict[str, Any]:
    ...
    try:
        ...
        # Format the state for agent consumption
        score = f"{state['home_team']['name']} {state['home_team']['score']}, {state['away_team']['name']} {state['away_team']['score']}"
        
        situation = f"{state['down']} and {state['distance']} at the {state['yard_line']}-yard line"
        
        clock = f"Q{state['quarter']} - {state['clock']}"
    ...
    try:
        ...
        if response.status_code != 200:
            return {
                "status": "ok",
                "found": False,
                "error_message": f"Player not found: {player_name}"
            }
...
2. **Functionality Tests**
   - Test each tool with mock service responses
   - Verify error handling for service unavailable

3. **Integration Tests**
   - Register tools with main agent
   - Invoke tools via agent and verify responses

### Success Criteria
- All tools register without errors
- Tools return properly formatted responses
- Agent can invoke tools and use results

## Files Modified

- `auto-madden/letta-tools/auto_madden_tools.py` (new)
- `auto-madden/letta-tools/requirements.txt` (new)
