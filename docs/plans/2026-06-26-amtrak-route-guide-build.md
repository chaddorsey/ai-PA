...
### Connective substrate → `route_connections.json` (BUILT)
`connect.py` grounds each Wikipedia node with: **categories** (Wikipedia cats, admin-filtered), **named_after** / **part_of** (Wikidata relations, resolved labels), and **dates** (Wikidata inception/dissolved/point-in-time + category-years + summary-years → the temporal axis). Per-leg the categories are aggregated into the recurring threads. **'Themes' is reserved for a future L4 pass** that synthesizes the broad threads driving a whole leg. (Outbound-link 1-hop crawl deferred to the interactive 'Why is that?' deep-dives.) The narrator weaves these CROSS-LAYER with the science/geo gradients from macro_context — the why/how lives at the seams (geology→extraction→settlement→bust; aridity→land-use→demographics) — aided by auto cross-layer hints + the `timeline.yml` national backbone for contemporaneity. Validated on Raton 2026-06-27. Earlier framing derived **threads** from categories — the trip-recurring threads (Santa Fe Trail · cattle frontier · coal & company towns · the Arkansas/Rio Grande corridors · Western Interior Seaway · aridity & the 100th meridian · the railroad's own story · Indigenous land & displacement · Dust Bowl & reclamation). Probed ✓: Santa Fe Trail → 196 links; Dodge City categories → "American frontier/Boot Hill/Gunsmoke/Arkansas River"; Wikidata → "named after" etc.
...
### Model audition (HARD GATE before any full narration run)
Before generating the full track/spines/cards, audition readers like hiring a narrator: the SAME representative segments (Raton dense-history+geology; a sparse desert; a big Cascades/Glacier transition; an urban approach) through the SAME packet across a PREMIUM slate (no cheap engines) — Claude Opus 4.8 + Sonnet (via subagent; proxy Anthropic credits exhausted), GPT-5.4 + GPT-5.5 (on proxy), Gemini latest (PROVISION before audition — proxy currently has only flash), Kimi 2.6 = kimi-k2p6 (on proxy) — presented side-by-side with per-segment tokens/cost + latency. (The on-train live-follow-up reader is a SEPARATE, smaller pick — a local model — not part of this build-time author audition.) User picks the author voice before any full run.
