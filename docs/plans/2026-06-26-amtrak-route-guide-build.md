...
Before generating the full track/spines/cards, audition readers like hiring a narrator: the SAME representative segments (Raton dense-history+geology; a sparse desert; a big Cascades/Glacier transition; an urban approach) through the SAME packet across a slate of models — Anthropic Claude Opus+Sonnet (subagent), OpenAI gpt-4.1 + gpt-4.1-mini, Google gemini-2.5-flash, deepseek-v4-pro, qwen3p6-plus — presented side-by-side with per-segment tokens/cost + latency. Note which voices stay good when small (the on-train live-follow-up reader must be a local model). User picks before any full run.
