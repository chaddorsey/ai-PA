...
- **R5.** Enable memfs. Memfs lives at `$HOME/.letta/agents/$AGENT_ID/memory/` per letta-code's resolver — **not** `~/.letta/memfs`. Strategy: bind-mount host `~/.letta/agents/` into the pa-web-ui container at the subprocess's `$HOME` equivalent path. Preserves MC's existing memory accumulated under LettaBot; survives container recreation.
