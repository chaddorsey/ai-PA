...
- Enumeration source = **`/Volumes/main-drive/ai-PA/data-resources/nyt_urls.txt`** — **5,618 bare URLs, one per line, already de-duplicated** (generated by `data-resources/extract-nyt-urls.py` from a saved-page HTML export).

> **SCALE NOTE (important):** 5,618 articles × 10–30s sequential pacing ≈ **16–47 hours** of wall-clock fetching, necessarily spread across many sessions (abort-on-block + resume). 5,618 authenticated page loads from one account is the real DataDome exposure here — so the smoke-gate (Task 7.3) and "stop on first block, resume hours later" discipline matter more than for a small set. Consider an initial **prioritized slice** (e.g. most recent N, or a topic filter) to validate value before committing to the full multi-day backfill.

### ⚠️ STATUS 2026-06-14 — PAUSED (DataDome blocks automated fetch)
Package fully built + tested (10 tests green); cookie-based session import works. **But the fetch is blocked:** with a valid imported `NYT-S` session + hardened real Chrome (`channel="chrome"`, automation flags stripped, `navigator.webdriver` hidden), the **first article returned 403 (DataDome)**. Root cause = DataDome fingerprints the Playwright/CDP automation layer itself (not the session), compounded by the home IP being in a temporary penalty box after ~4 bot-checks tripped during login attempts. Account is unaffected (IP/session-level block only). **Decision: paused to let the IP cool down.**
**Resume plan (next attempt, after cooldown):** try **CDP-attach to the user's real, already-working Chrome** — launch normal Chrome with `--remote-debugging-port=9222` (real fingerprint + real datadome token + real session that browses NYT fine), `playwright.chromium.connect_over_cdp(...)`, drive it slowly. If that still blocks, the only remaining lever is patched-Playwright (patchright/rebrowser) — explicit evasion, needs user consent. Built artifacts (urls/extract/frontmatter/state/block/cookies/fetch + login.sh/import-cookies.sh) are all reusable; only the browser-acquisition front-end changes.
...
- [ ] **Step 1: URL-list file (already located + verified)**

Path: **`/Volumes/main-drive/ai-PA/data-resources/nyt_urls.txt`** — 5,618 bare URLs, one per line, already unique. Confirmed format = plain `https://www.nytimes.com/...` per line (incl. some `/interactive/` URLs). Used as `--urls`. Re-verify anytime: `wc -l /Volumes/main-drive/ai-PA/data-resources/nyt_urls.txt`.

> `/interactive/` URLs render differently from standard articles — `extract.py` will raise "no body" on many and they'll be skipped (logged), which is fine. If you want them, that's a follow-up extractor.
...
cd /Volumes/main-drive/ai-PA/nyt-saved-archiver
./run-nyt-archive.sh /Volumes/main-drive/ai-PA/data-resources/nyt_urls.txt 2
```
...
Run: `./run-nyt-archive.sh /Volumes/main-drive/ai-PA/data-resources/nyt_urls.txt`
(Or a paced slice first, e.g. `... 200` to cap this session.)
Behavior: sequential, 10–30s apart, auto-skips cached, **auto-stops on any block**. At 5,618 URLs this spans ~16–47h across many resumed sessions — that's intended. If it stops on a block, wait several hours and re-run (state resumes where it left off).
