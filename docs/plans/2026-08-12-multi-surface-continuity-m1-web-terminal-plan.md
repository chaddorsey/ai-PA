...
- **⚠️ CORRECTED 2026-08-13 — the premise below is FACTUALLY WRONG.** The server broadcasts each
  approval to *every* subscribed connection and settles the race itself (`settled` guard in
  `requestApprovalOverWS`; the loser is answered "Approval request is no longer pending"). So the
  duplicate-response concern this decision is built on **does not exist**, and gating a response on
  "the injecting client only" could produce the one outcome that does matter — nobody answering.
  The shipped policy is: prevent via `exclude_interactive_tools`, and answer unconditionally when
  an approval arrives. Also, the request is a top-level `control_request` frame, not
  `approval_request_message`, and `approval_send` is not a server command. See
  `docs/plans/2026-08-13-approval-contract-findings.md`. Original decision retained below for
  provenance.
...
- [ ] **Unit 5: Terminal client (greenfield, text-first)** — **REOPENED 2026-08-13.** The terminal
  renders correctly and the client is built, but a nine-persona review found the safety property
...
- Create: `~/bin/letta-continuity` wrapper — **as built this is a TRACKED reference copy**
  (`clients/letta-terminal/bin/letta-continuity`) installed to `~/bin/`; it `cd`s to
  `$LETTA_LAUNCH_DIR` and execs the pinned local `tsx`. It deliberately **sources NO credentials**:
  this is a WS client, not an agent runtime, and credentials belong to the App Server process via
  `build_runtime_env`. **Unit 6 must carry the same rule forward** — do not source the agent
  credential set into a browser-facing bridge.
...
**Patterns to follow:** existing `~/bin/letta-<slug>` wrapper conventions for **cwd only** —
explicitly NOT creds sourcing (see Files above) — pointing at the client, not `letta --backend local`.
